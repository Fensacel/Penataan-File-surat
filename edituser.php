...
$result = $koneksi->query("SELECT * FROM penguna WHERE id = $id");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $koneksi->prepare("UPDATE penguna SET nis=?, nama=?, kelas=? WHERE id=?");
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>
                alert('Gagal memperbarui data user: " . htmlspecialchars($stmt->error) . "');
              </script>";
    }
...
<div class="page-wrapper">
    ...
    <form method="post">
        <div class="mb-3">
            <label>NIS</label>
            <input type="text" class="form-control" name="nis" required value="<?= htmlspecialchars($data['nis']) ?>">
        </div>
        <div class="mb-3">
            <label>Nama</label>
            <input type="text" class="form-control" name="nama" required value="<?= htmlspecialchars($data['nama']) ?>">
        </div>
        <div class="mb-3">
            <label>Kelas</label>
            <select name="kelas" class="form-control" required>
                <option value="XI PPLG 1" <?= $data['kelas'] == 'XI PPLG 1' ? 'selected' : '' ?>>XI PPLG 1</option>
                <option value="XI PPLG 2" <?= $data['kelas'] == 'XI PPLG 2' ? 'selected' : '' ?>>XI PPLG 2</option>
            </select>
        </div>
